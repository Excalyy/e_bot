...
class SQLiteDatabase:
    ...
    async def connect(self) -> None:
        ...
        if not self._is_connected or self._db is None:
            ...
            self._db.row_factory = aiosqlite.Row  # Для доступа по имени столбцов
            self._is_connected = True
            await self._create_tables()
            print(f"✅ Подключение к SQLite установлено: {self._db_path}")
    
    def _ensure_connected(self) -> aiosqlite.Connection:
        """
        Проверяет, что подключение установлено.
        
        Returns:
            aiosqlite.Connection: Активное подключение
            
        Raises:
            ConnectionError: Если подключение не установлено
        """
